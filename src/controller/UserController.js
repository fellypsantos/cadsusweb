...
module.exports = {
  async addUser(req, res) {
    ...
    if (duplicated === null) {
      ...
      console.log(
        "Cadastro já existe na base offline, atualizando com os dados recebidos."
      );

      const response = await User.findByIdAndUpdate(duplicated._id, req.body, {
        new: true,
  ...
  async generateCardPastingData(req, res) {
    const jsonData = JSON.parse(req.params.jsondata);

    const cns = jsonData.numeroCNS;

    const userData = {
      nome: jsonData.Nome,
      numeroCns: formattedCns(cns),
      dataNascimento: jsonData.dataNascimento.split("-").reverse().join("/"),
      sexo: jsonData.codigoSexo,
      municipioNascimentoCodigo: jsonData.codigoMunicipio,
    };
    ...
    res.render("cartao", { cardList: [userData] });
  },
};
